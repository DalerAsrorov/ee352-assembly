...
createNewLoop:
	lw $t6, 0($a3)
	bltz $t6, identifyEdge	#branch to the edge if the value is less than 0. If not found (final point).
  	sw $t6, 0($a2)  
  	addi $a2, $a2, 4 	#storing the data to appropriate register
  	addi $a3, $a3, 4	#storing the data to appropriate register
  	
  	#launch the loop 
  	#to store the values
	j createNewLoop		#jumps to the loop
      	
identifyEdge:
 	sw $a2, stackPointer #save the stackPointer to a2
	jr $ra		     #returns	
